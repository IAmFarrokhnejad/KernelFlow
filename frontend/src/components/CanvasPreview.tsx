...
export default forwardRef<CanvasPreviewHandle, Props>(function CanvasPreview({ imageUrl, imageBlob }, ref) {
  ...
  const applyFilter = async (filter: string, isSimple: boolean, kernel?: Kernel3x3, recordId: number = 0) => {
    ...
    try {
      if (useClientPath) {
        // ── Client-side pixel loop (now true per-row raster with more delay) ───────────────────────────────
        ...
        for (let y = 0; y < h; y++) {
          for (let x = 0; x < w; x++) {
            ...
            if (filter === "gaussian") {
              ...
              for (let ky = -1; ky <= 1; ky++) for (let kx = -1; kx <= 1; kx++) {
                ...
                const ii = (py2 * w + px) * 4, wt = gaussKernel[ky + 1][kx + 1];
                r += src[ii] * wt; g += src[ii + 1] * wt; b += src[ii + 2] * wt; sum += wt;
              }
          ...
          // ← NEW: update after every row (true raster scan) + more delay
        ...
        // ── Server SSE path — now uses correct record_id and progressive per-row reveal
        if (!imageBlob) return;
        const form = new FormData();
        form.append("file", imageBlob, "image");
        form.append("params", JSON.stringify({ kernel, boost: 1.8 }));
        form.append("record_id", recordId.toString());   // ← FIXED: correct record ID
