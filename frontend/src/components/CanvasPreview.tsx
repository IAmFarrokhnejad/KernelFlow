...
const applyFilter = async (filter: string, isSimple: boolean, kernel?: Kernel3x3) => {
  ...
  try {
    if (useClientPath) {
      ...
      // ── Server SSE path — used for ALL other filters ──────────────────────
      // ✅ Reached when: checkbox is off, OR filter has no client implementation
      if (!imageBlob) return;
      const form = new FormData();
      form.append("file", imageBlob, "image");
      form.append("params", JSON.stringify({ kernel, boost: 1.8 }));
      form.append("record_id", "0");
